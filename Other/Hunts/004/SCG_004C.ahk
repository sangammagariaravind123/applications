...
start() {
    if (ImageSearch(&SCG2X, &SCG2Y, 0, 0, A_ScreenWidth, A_ScreenHeight, "SCG.png")) {
        Click(SCG2X, SCG2Y)
        Sleep(1000)
        Click(SCG2X, SCG2Y)
    }
    if (ImageSearch(&SCGX, &SCGY, 0, 0, A_ScreenWidth, A_ScreenHeight, "SCG2.png")) {
        Click(SCGX, SCGY)
    }
    ...
    if (ImageSearch(&next2X, &next2Y, 0, 0, A_ScreenWidth, A_ScreenHeight, "next2.png")) {
        ...
        loop 16 {
            Send "{d}"
            Sleep(100)
            if (ImageSearch(&SCGX, &SCGY, 0, 0, A_ScreenWidth, A_ScreenHeight, "SCG.png")) {
                Click(SCGX, SCGY)
            }
            else if (ImageSearch(&SCG2X, &SCG2Y, 0, 0, A_ScreenWidth, A_ScreenHeight, "SCG2.png")) {
                Click(SCG2X, SCG2Y)
            }
        }
    }
...
play() {
    ...
    Sleep(1600)
    Send("{s down}") ;4200
    ToolTip "Start drifting ... turn left"
    SetTimer () => ToolTip(), -1500
    Sleep(1700)
    Send("{a}") ;5900
    Sleep(1500)
    ToolTip "Stop drifting and shockwave"
    SetTimer () => ToolTip(), -1500
    Send("{s up}") ;7400
    Send("{Space}")
    Send("{Space}")
